...
### Documentation
- The bundled TicketSQL grammar covers RT 6.0.3. RT 6.0.3 added searching on the custom fields of a ticket's role members — `Owner.CustomField.Manager = 'jane'`, `Requestor.CustomField.{Department} = 'Sales'`, and the same through a custom role — which the reference did not mention, so an AI on a 6.0.3 instance never reached for it. The new section documents the syntax, the operators, and the fact that a name with spaces needs the whole left-hand side quoted rather than just the braces. It also records two failures RT reports no error for: an unknown or unreadable custom field matches nothing, while an unknown custom role makes the condition vanish, leaving the query to return whatever its remaining clauses match.
- The `create-queue` prompt says what to do with a lifecycle transition right. It already explained the lifecycle `rights` map and RT's ModifyTicket/DeleteTicket fallback, but not that the right it names has to be granted like any other — an approval gate whose right is granted to nobody stops everyone, including the approver. The prompt now says where the right shows up (`get_available_rights` under `Status`, on that queue, once the lifecycle exists) and that it belongs to the approving group alone.
...
- `list_group_members` says what RT actually returns. That collection ignores a `fields` parameter, so it yields only an id and a type per member; the description now says so and points at `get_group`, whose `Members` resolve a user member to its username and a group member to its numeric id.
- Two administration tool descriptions corrected against a live RT. `get_available_rights` listed General, Admin and Status as though the set were fixed. All three are real and `Staff` was missing, but the set varies by object: a group offers only Admin and Staff, and `Status` appears on a queue — and globally — only once a lifecycle reserves a status transition behind a named right, which RT registers there via `RT::Queue->AddRight`. That is where a custom sign-off right shows up, so the description now names all four, says the set depends on the object, and tells the AI to read the categories back from the response. `update_lifecycle` said RT backfills a missing `defaults.on_create` and falls back to `ModifyTicket`/`DeleteTicket` for absent rights, which it does, but in memory as it loads the config: neither key reappears in `get_lifecycle`, so the description now says an absence there is a working default rather than damage to repair.
