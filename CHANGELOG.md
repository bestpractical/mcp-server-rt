...
### Changed
- `update_ticket` no longer accepts a bare link relation such as `RefersTo`. Setting one syncs that relation to exactly the value given and silently removes every other link of the type, which is a destructive default when an AI is asked to add a link. Links are now changed only through `AddRefersTo`/`DeleteRefersTo` and the equivalents for `ReferredToBy`, `DependsOn`, `DependedOnBy`, `Parent`, and `Child`; each takes one ID, an array of IDs, or an external URI. Passing a bare relation — including RT aliases like `Children` or `MemberOf` — is refused with a message naming the incremental fields, and no links are changed. To replace a link, delete the old one and add the new one. `create_ticket` is unaffected, since a new ticket has no links to remove.
