...
statuses. This is optional — the admin can skip it and add colors
later via Admin > Lifecycles. If they want colors, suggest sensible
defaults based on the status meaning (e.g., blue for initial/active,
amber for waiting/blocked, green for completed, red for denied,
gray for withdrawn/deleted). Set colors via `update_lifecycle` using
the `colors` parameter: `{"status_name": "#hex_color"}`.
...
Once the conversation has enough detail to proceed, confirm the plan
with the admin before making any changes.

1. **Check existing lifecycles**: `list_lifecycles`. If a custom
   lifecycle is needed, `create_lifecycle` (optionally cloning one)
   then `update_lifecycle`.
